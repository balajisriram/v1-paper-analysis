...
def load(filepath):
    ...
    if 'continuous' in filepath.lower():
        data = loadContinuous(filepath)
    elif 'spikes' in filepath.lower():
        data = loadSpikes(filepath)
    elif 'events' in filepath.lower():
...
def pack_2(folderpath, destination=None, filename = '', channels = 'all', chprefix = 'CH', 
    ...
    '''
    if not destination:
        print('destination is set to the source path')
    ...
    if not dref:
        ...
        if not filename: filename = source + '_' + chprefix + 's' + '_' + dref_prefix + session + '.dat'
    print('Packing data to file: ' + filename + ' at location ' + destination)
    data_array.tofile(os.path.join(destination,filename))
